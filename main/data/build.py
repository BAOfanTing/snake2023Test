...
#             root = os.path.join(config.DATA.DATA_PATH, prefix)
        ...
        dataset = DatasetMeta(root=root,transform=transform,train=is_train,aux_info=config.DATA.ADD_META,dataset=config.DATA.DATASET)
        nb_classes = 100
    elif config.DATA.DATASET == 'snakeclef2023':
        ...
        nb_classes = 1784
    elif config.DATA.DATASET == 'snakeclef2023trainvalall':
        # root = './datasets/snakeclef2023'
        root = '.../data'
        dataset = DatasetMeta(root=root,transform=transform,train=is_train,aux_info=config.DATA.ADD_META,dataset=config.DATA.DATASET,over_sampling=config.OVER_SAMPLING)
        nb_classes = 1784
    elif config.DATA.DATASET == 'snakeclef2023test':
        root = '../data'
        dataset = DatasetMeta(root=root,transform=transform,train=is_train,aux_info=config.DATA.ADD_META,dataset=config.DATA.DATASET)
        nb_classes = 1784
    elif config.DATA.DATASET == 'snakeclef2023val':
        root = '/root/autodl-tmp/data'
        dataset = DatasetMeta(root=root,transform=transform,train=is_train,aux_info=config.DATA.ADD_META,dataset=config.DATA.DATASET)
        nb_classes = 1784
    else:
...
def build_transform(is_train, config):
    ...
    if resize_im:
        if config.TEST.CROP:
            ...
            t.append(
                transforms.Resize(size,str_to_pil_interp(config.DATA.INTERPOLATION)) #, interpolation=transforms.InterpolationMode.BICUBIC),
                # to maintain same ratio w.r.t. 224 images
            )
            #5.19改变
            #t.append(transforms.CenterCrop(512))
            t.append(transforms.CenterCrop(config.DATA.IMG_SIZE))
        else:
            t.append(
                transforms.Resize((config.DATA.IMG_SIZE, config.DATA.IMG_SIZE), str_to_pil_interp(config.DATA.INTERPOLATION))#,
...
########## 
    ...
    if is_train:
        ...
        t.append(transforms.RandomRotation(45))
############
