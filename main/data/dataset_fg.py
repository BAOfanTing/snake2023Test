...
def get_statistics_on_trainval(root):
    items = load_csv_annotations(fp=os.path.join(root, 'SnakeCLEF2023-Train.csv'))
    classes = set()
...
def load_statistics_from_file(root):
    ...
    return classes, endemics, location_codes, class_to_idx, cls_hist, classes_dist


def make_meta_data(location_codes, endemics, it):
    # location codes
    ...
    # concat
    meta_data = meta_data1 + meta_data3
    return meta_data

# here
def find_images_and_targets_snakeclef2023(root,dataset, istrain=False, aux_info=False, over_sampling=False):
    classes, endemics, location_codes, class_to_idx, cls_hist, classes_dist = load_statistics_from_file(root)

    sorted_hist = sorted(cls_hist.values())
    idx_ref = get_hist_percent_index(sorted_hist, percent=0.1)
    num_ref = sorted_hist[idx_ref]

    imgs_root = os.path.join(root, 'SnakeCLEF2023-small_size')
    # print("### huazhen: current imgs_root:{}".format(imgs_root))
    if istrain:
        data_file = os.path.join(root,'train_split.csv')
    else:
        data_file = os.path.join(root,'valid_split.csv')

    # print("### huazhen: current data_file:{}".format(data_file))
    # items = load_csv_annotations(fp=data_file)
    items = load_csv_annotations(fp=data_file, row_length=6)  # fixed 20230313
    # print("### huazhen: current len(items):{}".format(len(items)))
    images_and_targets = []
    images_info = []
    for it in items:
        file_path = os.path.join(imgs_root, it['image_path'])
        ...
        for rpt in range(t_coef):
            if aux_info:
                meta_data = make_meta_data(location_codes, endemics, it)
                images_and_targets.append([file_path, target, meta_data])
            else:
                images_and_targets.append([file_path, target])
    return images_and_targets, class_to_idx, images_info

def find_images_and_targets_snakeclef2023_trainvalall(root,dataset,istrain=False,aux_info=False,over_sampling=False):
    classes, endemics, location_codes, class_to_idx, cls_hist, classes_dist = load_statistics_from_file(root)
    ...
    imgs_root_train = os.path.join(root, 'SnakeCLEF2023-small_size')
    imgs_root_test = os.path.join(root, 'SnakeCLEF2023-test_images')
    if istrain:
        data_file_list = [
    ...
    images_info = []
    for it in items_all:
        file_path = os.path.join(it['imgs_root'], it['image_path'])
        class_name = it['class_id']
        target = class_to_idx[class_name]
...
def find_images_and_targets_snakeclef2023_for_OJ(root,dataset,istrain=False,aux_info=False):
    classes, endemics, location_codes, class_to_idx, cls_hist, classes_dist = load_statistics_from_file(root)

    imgs_root = os.path.join(root, 'SnakeCLEF2023-test_images')
    data_file = os.path.join(root, 'SnakeCLEF2023-Test.csv')
...
def find_images_and_targets_snakeclef2023_for_OJvalid(root,dataset,istrain=False,aux_info=False):
    classes, endemics, location_codes, class_to_idx, cls_hist, classes_dist = load_statistics_from_file(root)
...
class DatasetMeta(data.Dataset):
    def __init__(
        ...
        elif dataset == 'snakeclef2023':  # here
            images,class_to_idx,images_info = find_images_and_targets_snakeclef2023(root,dataset,train,aux_info,over_sampling)
        elif dataset == 'snakeclef2023trainvalall':
            images,class_to_idx,images_info = find_images_and_targets_snakeclef2023_trainvalall(root,dataset,train,aux_info,over_sampling)
        elif dataset == 'snakeclef2023test':
            images,class_to_idx,images_info = find_images_and_targets_snakeclef2023_for_OJ(root,dataset,train,aux_info)
        elif dataset == 'snakeclef2023valid':
            images,class_to_idx,images_info = find_images_and_targets_snakeclef2023_for_OJvalid(root,dataset,train,aux_info)
        if len(images) == 0:
        ...
        # self.scale = transforms.Scale(384)  # https://pytorch.org/vision/0.9/transforms.html#torchvision.transforms.Scale
        self.scale = transforms.Resize(384)  # fixed 20230313
